...
This seems like an alternative which is at least as ergonomic if not more than
expected-only alternative.
...
<pre>
T val = get_value();
if (auto result = q.try_push(std::move(tmp)))
  return;
else
